...
$(document).ready(function () {
    var baseUrl = 'http://localhost:4523/m1/2933120-0-default';
    $.ajaxSetup({
        headers: {
    ...
    $.getJSON("/schedule", function (schedule) {
        ...
        $.each(schedule.jobList, (index, job) => {
            ...
            if (job.crew == null || job.startDate == null) {
                ...
                const beforeReady = moment(job.startDate).isBefore(moment(job.readyDate));
                const afterDue = moment(job.endDate).isAfter(moment(job.dueDate));
